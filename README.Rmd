...
* Skip first 18+n bytes when serializing - this is all very speific information
  like R_VERSION number which would change on every minor version bump.
  See `rlang::hash()` implementation for details.
...
`base::serialize()`.
* `xxhash(robj, algo, seed)` calculates the hash of a raw vector or string. 
